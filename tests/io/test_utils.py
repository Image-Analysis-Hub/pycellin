...
def test_update_node_prop_key():
    """Test updating a node property key."""
    lineage = CellLineage()
...
def test_update_node_prop_key_missing_old_key_skip():
...
def test_update_node_prop_key_enforce_old_key_existence():
    """Test that missing old_key raises error when enforce_old_key_existence=True."""
    lineage = CellLineage()
    lineage.add_node(1, old_key="value1")
    lineage.add_node(2)  # No old_key

    with pytest.raises(ValueError, match="Node 2 does not have the required key 'old_key'"):
        _update_node_prop_key(lineage, "old_key", "new_key", enforce_old_key_existence=True)


def test_update_node_prop_key_set_default_if_missing():
...
def test_update_node_prop_key_set_default_none():
    """Test setting None as default value when old_key is missing."""
    lineage = CellLineage()
    lineage.add_node(1, old_key="value1")
    lineage.add_node(2)  # No old_key

    _update_node_prop_key(lineage, "old_key", "new_key", set_default_if_missing=True)

    assert lineage.nodes[1]["new_key"] == "value1"
    assert lineage.nodes[2]["new_key"] is None


def test_update_node_prop_key_empty_lineage():
...
def test_update_node_prop_key_same_key_name():
...
def test_update_lineage_prop_key():
    lineage = CellLineage()
...
def test_add_lineages_props_no_lin_ID_on_one_node():
    g1_attr = {"name": "blob", "lineage_ID": 0}
    g2_attr = {"name": "blub", "lineage_ID": 1}
...
def test_add_lineages_props_different_ID_for_one_track():
    g1_attr = {"name": "blob", "lineage_ID": 0}
    g2_attr = {"name": "blub", "lineage_ID": 1}
...
def test_add_lineages_props_no_nodes():
    g1_attr = {"name": "blob", "lineage_ID": 0}
    g2_attr = {"name": "blub", "lineage_ID": 1}
